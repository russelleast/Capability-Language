import { CapabilitySummary, SemanticSummary } from "../views/semanticSummary";
import { displayNameForGraph } from "./DclGraphLabels";
import { DclGraphEdge, DclGraphEdgeReason, DclGraphModel, DclGraphNode } from "./DclGraphModel";
import { semanticIdentity } from "./DclSemanticIdentity";

export const CAPABILITY_INFLUENCE_GRAPH_DESCRIPTION = "The Capability Influence Graph shows explicit compiler-derived semantic influence between capabilities. It does not show all dependencies, shared ownership, similarity, or implementation coupling.";

type InfluenceKind = "lifecycle" | "event" | "relation" | "effect" | "participation" | "policy" | "context reference";

const SCORE_BY_KIND: Record<InfluenceKind, number> = {
  lifecycle: 5,
  event: 4,
  relation: 3,
  effect: 3,
...
export function buildCapabilityInfluenceGraph(summary: SemanticSummary): DclGraphModel | undefined {
  if (summary.capabilities.length < 2) return undefined;

  const resolver = capabilityResolver(summary.capabilities);
  const nodes = summary.capabilities.map(capabilityNode);
  const influences: PendingInfluence[] = [];
  ...
  return {
    ...
    description: CAPABILITY_INFLUENCE_GRAPH_DESCRIPTION,
...
function collectLifecycleInfluence(
  capabilities: CapabilitySummary[],
  resolve: (value: string | undefined) => CapabilitySummary | undefined,
  influences: PendingInfluence[],
): void {
  for (const target of capabilities) {
    for (const transition of target.lifecycle?.transitionDetails ?? []) {
      ...
      if (!source || source.name === target.name) continue;
      influences.push({
        source: source.name,
        target: target.name,
        kind: "lifecycle",
...
function collectEventInfluence(
  capabilities: CapabilitySummary[],
  resolve: (value: string | undefined) => CapabilitySummary | undefined,
  influences: PendingInfluence[],
  warnings: string[],
): void {
  const consumersByEvent = new Map<string, CapabilitySummary[]>();
  ...
  for (const emitter of capabilities) {
    for (const emission of emitter.eventDetails ?? []) {
      ...
      for (const consumer of consumers) {
        ...
        if (!target || target.name === emitter.name) continue;
        influences.push({
          source: emitter.name,
          target: target.name,
          kind: "event",
...
function collectRelationInfluence(
  capabilities: CapabilitySummary[],
  resolve: (value: string | undefined) => CapabilitySummary | undefined,
  influences: PendingInfluence[],
): void {
...
function collectEffectInfluence(
  capabilities: CapabilitySummary[],
  resolve: (value: string | undefined) => CapabilitySummary | undefined,
  influences: PendingInfluence[],
): void {
  for (const source of capabilities) {
    for (const effect of source.effectDetails ?? []) {
      if (effect.targetKind !== "capability") continue;
...
function collectParticipationInfluence(
  capabilities: CapabilitySummary[],
  resolve: (value: string | undefined) => CapabilitySummary | undefined,
  influences: PendingInfluence[],
): void {
  for (const owner of capabilities) {
    ...
    for (const participantName of participants) {
      ...
      if (!participant || participant.name === owner.name) continue;
      influences.push({
...
function collectPolicyInfluence(
  capabilities: CapabilitySummary[],
  resolve: (value: string | undefined) => CapabilitySummary | undefined,
  influences: PendingInfluence[],
): void {
  for (const source of capabilities) {
    for (const policy of source.policyDetails ?? []) {
      if (policy.targetKind !== "capability") continue;
...
function collectContextReferenceInfluence(
...
function aggregateInfluences(influences: PendingInfluence[]): DclGraphEdge[] {
  const grouped = new Map<string, PendingInfluence[]>();
  ...
  return Array.from(grouped.values()).map((items) => {
    ...
    const reasons = items.map(reasonForInfluence).sort((a, b) => (
      b.score - a.score
...
function reasonForInfluence(influence: PendingInfluence): DclGraphEdgeReason {
  return {
    kind: influence.kind,
...
function capabilityResolver(capabilities: CapabilitySummary[]): (value: string | undefined) => CapabilitySummary | undefined {
  const byName = new Map<string, CapabilitySummary>();
  const byId = new Map<string, CapabilitySummary>();
  const byQualifiedName = new Map<string, CapabilitySummary>();
  for (const capability of capabilities) {
    byName.set(capability.name, capability);
    if (capability.id) byId.set(capability.id, capability);
    if (capability.context) byQualifiedName.set(`${capability.context}.${capability.name}`, capability);
  }
