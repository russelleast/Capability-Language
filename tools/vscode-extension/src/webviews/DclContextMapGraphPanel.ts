...
export class DclContextMapGraphPanel {
  private static currentPanel: vscode.WebviewPanel | undefined;
  private static currentGraph: DclGraphModel | undefined;

  static show(extensionUri: vscode.Uri, graph: DclGraphModel, selectedContext?: string): void {
  ...
  private static async handleMessage(message: unknown): Promise<void> {
    if (!isContextMapMessage(message)) return;
...
function renderHtml(webview: vscode.Webview, extensionUri: vscode.Uri, graph: DclGraphModel, selectedContext: string | undefined): string {
...
function isContextMapMessage(message: unknown): message is ContextMapMessage {
  if (!message || typeof message !== "object") return false;
  const candidate = message as Partial<ContextMapMessage>;
