...
export class DclEventFlowGraphPanel {
  private static currentPanel: vscode.WebviewPanel | undefined;
  private static currentGraph: DclGraphModel | undefined;

  static show(extensionUri: vscode.Uri, graph: DclGraphModel, selectedEvent?: string): void {
  ...
  private static async handleMessage(message: unknown): Promise<void> {
    if (!isEventFlowGraphMessage(message)) return;
...
function renderHtml(webview: vscode.Webview, extensionUri: vscode.Uri, graph: DclGraphModel, selectedEvent: string | undefined): string {
...
function isEventFlowGraphMessage(message: unknown): message is EventFlowGraphMessage {
  if (!message || typeof message !== "object") return false;
  const candidate = message as Partial<EventFlowGraphMessage>;
